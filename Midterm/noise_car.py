...
def create_body(length=2, width=1):
    ...
    body = cmds.polyPlane(w=length, h=width, subdivisionsX=200, subdivisionsY=200, name="body")
...
def create_tire(name, width, radius, tx, ty, tz):
    ...
    # Return the transform node name.
    tire = cmds.polyCylinder(h=width, r=radius, ax=(0,0,1), sc=True, subdivisionsX=200, subdivisionsY=200, name=name)
    # start the timer and add the noise
    timeStart = time.clock()
